...
const sendMailAfterPayment = () => {
    ...
    userData.planSelected = "YouTube Video Editing Sesssion"
  firebase.auth().onAuthStateChanged(user => {
    ...
    const apiUrl = "https://rs-server.onrender.com/v1/user/sendMailWithPurchase";
    // const apiUrl = "http://localhost:3000/v1/user/sendMailWithPurchase";
    axios.post(apiUrl, userData).then(async (response) => {
    // $(".loader-wrapper").css("display", "none");
        if (response.data) {
            window.location = "../thank-you-video-editing-session.html";
        } else {
