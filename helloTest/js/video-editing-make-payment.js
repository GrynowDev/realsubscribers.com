...
const sendMailAfterPayment = () => {
  ...
  const userData = user
  const apiUrl =
      'https://rs-server.onrender.com' + user.serviceUrl
  // const apiUrl = "http://localhost:3100/v1/ve/sendPaymentMail";
...
const doPayment = async (token, userInfo) => {
  ...
  const apiUrl = 'https://rs-server.onrender.com/v1/doPayment'
  axios
...
function getUser() {
  const userPlanDetails = JSON.parse(localStorage.getItem('userPlanDetails'))
  user = JSON.parse(localStorage.getItem('user'))
  user.planSelected = userPlanDetails.selectedPlan
  user.serviceName = userPlanDetails.serviceName
  user.totalAmount = userPlanDetails.totalAmount
...
function bindValueToUI() {
  ...
  if (user) {
    ...
    document.getElementById('plan-selected').textContent = user.selectedPlan
