...
let totalAmount = 10,
    isApplied = 0
discount = 0
serviceName = 'Buy Youtube Views'
const serviceUrl = '/v1/rs/sendPaymentMailForYtViews'
const user = {}
...
const setUserData = (user) => {
    user.quantity = user.selectedYoutubeViews + ' views'
...
function addUser() {
    ...
    const userInfo = user
    axios
        .post(apiUrl + '/v1/rs/addUserForYtViews', userInfo)
        .then((res) => {
            axios
                .post(apiUrl + '/v1/rs/sendYtViewsLead', userInfo)
                .then((res) => {
                    resetSpinner()
...
const selectPlan = (selectedPlan) => {
    ...
    totalAmount = element.value * 0.01
    $('#price').html(totalAmount)
    applyPromoCode()
