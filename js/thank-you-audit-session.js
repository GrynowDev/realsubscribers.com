...
const sendMailToGetService = async() => {
    var isMailSent = localStorage.getItem("audit-session-mail-sended");
    if (isMailSent) {
    ...
    await firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            userData.planSelected = "Youtube Channel Technical Audit Session";
            axios.post(apiUrl, userData).then(async(response) => {
                if (response.data) {
                    $(".loader-wrapper").css("display", "none");
                    localStorage.setItem("navigatedFrom", "home-auth");
                    alert("Mail sent successfully");
                    localStorage.setItem("audit-session-mail-sended", true);
...
const resetMailStatus = (requestedFrom) => {
    localStorage.removeItem("audit-session-mail-sended");
